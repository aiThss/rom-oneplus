...
export function publicOrigin() {
  ...
  if (!raw)
    throw new Error(
      'Set PUBLIC_ORIGIN to the public HTTPS origin of this site.',
    );
...
export function migrate(database, migrationsDirectory) {
  database.exec(
    'CREATE TABLE IF NOT EXISTS rom_migrations (name TEXT PRIMARY KEY, digest TEXT NOT NULL, applied_at INTEGER NOT NULL)',
  );
  for (const name of readdirSync(migrationsDirectory)
    ...
    const applied = database
      .prepare('SELECT digest FROM rom_migrations WHERE name=?')
      .get(name);
    if (applied) {
    ...
    try {
      ...
      database
        .prepare(
          'INSERT INTO rom_migrations(name,digest,applied_at) VALUES(?,?,?)',
        )
...
export function provisionAdmin(database, { reset = false } = {}) {
  if (database.prepare('SELECT id FROM admin WHERE id=1').get() && !reset)
    return false;
  const username = process.env.ADMIN_USERNAME || 'admin';
  const password = process.env.ADMIN_PASSWORD || '';
  if (!/^[a-zA-Z0-9_.-]{3,60}$/.test(username))
    throw new Error(
      'ADMIN_USERNAME must contain 3–60 letters, digits, _, . or -.',
    );
  if (password.length < 14 || password.length > 200)
    throw new Error(
      'Set ADMIN_PASSWORD to a unique password of 14–200 characters before the first start.',
    );
  ...
  try {
    database
      .prepare(
        'INSERT INTO admin(id,username,salt,hash) VALUES(1,?,?,?) ON CONFLICT(id) DO UPDATE SET username=excluded.username,salt=excluded.salt,hash=excluded.hash',
      )
      .run(username, salt, hash);
    database.exec(
      'DELETE FROM sessions; DELETE FROM login_throttle; DELETE FROM login_throttle_bucket; COMMIT;',
    );
  } catch (error) {
    database.exec('ROLLBACK');
    throw error;
...
export function databaseAdapter(database) {
  return { prepare: (sql) => new PreparedQuery(database, sql) };
}
  ...
  return {
    async get(key) {
      ...
      try {
        ...
        const contentType = key.endsWith('.jpg')
          ? 'image/jpeg'
          : key.endsWith('.webp')
            ? 'image/webp'
            : 'image/png';
